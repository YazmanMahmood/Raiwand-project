...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Firebase initialization and data fetching for Node 2
  const temperatureRef = ref(database, 'bay 1/node 2/temperature');
  const soilMoistureRef = ref(database, 'bay 1/node 2/soil_moisture');
  const humidityRef = ref(database, 'bay 1/node 2/humidity');
  ...
  // Control panel functionality for Node 2
  ...
  waterPumpDropdown.querySelector('select').addEventListener('change', (event) => {
    ...
    // Implement logic to control the water pump for Node 2
    console.log('Water Pump Node 2:', selectedValue);
  });

  fansDropdown.querySelector('select').addEventListener('change', (event) => {
    const selectedValue = event.target.value;
    // Implement logic to control the fans for Node 2
    console.log('Fans Node 2:', selectedValue);
  });
});
